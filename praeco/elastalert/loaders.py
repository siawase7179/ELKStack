...
class RulesLoader(object):
    ...
    def load_configuration(self, filename, conf, args=None):
        ...
        elastalert_logger.info('filename : %s' % filename)
        elastalert_logger.info('rule : %s' % rule)
        elastalert_logger.info('conf : %s' % conf)
    ...
    def load_modules(self, rule, args=None):
        ...
        for enhancement_name in rule.get('match_enhancements', []):
            elastalert_logger.info('enhancement_name: %s' % enhancement_name)
