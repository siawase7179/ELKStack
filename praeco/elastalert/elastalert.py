...
class ElastAlerter(object):
    ...
    def create_index(self):
        esversion = get_version_from_cluster_info(self.writeback_es)
        es_index_mappings = {}
        if self.is_atleasteight(esversion):
            es_index_mappings = self.read_es_index_mappings()
        elif self.is_atleastseven(esversion):
            es_index_mappings = self.read_es_index_mappings(7)
        else:                                      
            elastalert_logger.error('FATAL - Unsupported Elasticsearch version: ' + esversion + '. Aborting.')
            exit(1)
        ...
        for index_name in index_names:
            if self.writeback_es.indices.exists(index_name):
                try:
                    self.writeback_es.indices.delete(index_name)
                except NotFoundError:
                    # Why does this ever occur?? It shouldn't. But it does.
                    pass
            self.writeback_es.indices.create(index_name)
        
        time.sleep(2)

        if self.is_atleasteight(esversion):
            self.writeback_es.indices.put_mapping(index=self.writeback_index,
                                      body=es_index_mappings['elastalert'])
        elif self.is_atleastseven(esversion):
            self.writeback_es.indices.put_mapping(index=self.writeback_index, doc_type='_doc',
                                        body=es_index_mappings['elastalert'], include_type_name=True)

        elastalert_logger.info('New index %s created' % self.writeback_index)

    def read_es_index_mapping(self, mapping, es_version=7):
        base_path = os.path.abspath(os.path.dirname(__file__))
        mapping_path = 'es_mappings/{0}/{1}.json'.format(es_version, mapping)
        path = os.path.join(base_path, mapping_path)
        with open(path, 'r') as f:
            elastalert_logger.info("Reading index mapping '{0}'".format(mapping_path))
            return json.load(f)
        
    def read_es_index_mappings(self, es_version=8):
        return {
            'elastalert': self.read_es_index_mapping('elastalert', es_version),
        }
    
    def is_atleastseven(self, es_version):
        return int(es_version.split(".")[0]) >= 7

    def is_atleasteight(self, es_version):
